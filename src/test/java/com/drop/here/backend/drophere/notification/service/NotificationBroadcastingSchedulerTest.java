...
class NotificationBroadcastingSchedulerTest extends IntegrationBaseClass {
    ...
    void prepare() {
        ...
        notificationTokenRepository.save(NotificationToken.builder().notificationTokenType(NotificationTokenType.CUSTOMER).token("token123").build());
    ...
    void givenNotSentNotificationsWhenSendThenSend() {
        ...
        notification.setBroadcastingStatus(NotificationBroadcastingStatus.NOT_SENT);
        ...
        //then
        assertThat(notificationRepository.findById(notification.getId()).orElseThrow().getBroadcastingStatus())
                .isEqualTo(NotificationBroadcastingStatus.SENT);
    }
