...
class RouteObsoleteRouteSchedulerTest extends IntegrationBaseClass {
    ...
    @Autowired
    private RouteObsoleteRouteScheduler routeObsoleteRouteScheduler;
    ...
    @Test
    void givenObsoleteRouteWhenFinishObsoleteThenFinish() {
        //given
        final Route route = RouteDataGenerator.route(1, company);
        route.setStatus(RouteStatus.ONGOING);
        route.setRouteDate(LocalDate.now().minusDays(1));
        ...
        //when
        routeObsoleteRouteScheduler.finishObsolete();
        ...
        assertThat(savedRoute.getCanceledAt()).isBetween(LocalDateTime.now().minusMinutes(1), LocalDateTime.now());
    }

    @Test
    void givenNotObsoleteRouteWhenFinishObsoleteThenDoNothing() {
        //given
        final Route route = RouteDataGenerator.route(1, company);
        route.setStatus(RouteStatus.ONGOING);
        ...
        //then
        final Route savedRoute = routeRepository.findById(route.getId()).orElseThrow();
        assertThat(savedRoute.getStatus()).isNotEqualTo(RouteStatus.FINISHED);
        assertThat(savedRoute.getCanceledAt()).isNull();
    }
