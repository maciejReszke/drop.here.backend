...
class DropFinishedUpdateServiceTest {
    ...
    void givenDropSpotAndRequestForceWhenUpdateThenUpdateAndSendNotifications() {
        ...
        when(spotMembershipService.findToBeNotified(spot, SpotMembershipNotificationStatus.finished())).thenReturn(List.of());

        //when
        final DropStatus result = dropFinishedUpdateService.update(drop, spot, company, accountProfile, dropManagementRequest, true);
    ...
    void givenDropSpotAndRequestNotForceWhenUpdateThenUpdateAndSendNotifications() {
        ...
        doNothing().when(dropValidationService).validateFinishedUpdate(drop);
        doNothing().when(notificationService).createNotifications(any());
        when(spotMembershipService.findToBeNotified(spot, SpotMembershipNotificationStatus.finished())).thenReturn(List.of());

        //when
        final DropStatus result = dropFinishedUpdateService.update(drop, spot, company, accountProfile, dropManagementRequest, false);
